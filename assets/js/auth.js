...
function userFlow() {
  ...
  //User has already Group Selection is done
  if (userStatus == 'groupSelected') {
   $("#userGroupSelect").hide();
    $("#userActivitySelect").show();

  }
  else if (userStatus == 'trueAndLiesSelected') {//User has also set True and Lies, Move to play game
    window.location.href = "submitScreen.html";
  }
  else {
    $("#userLogin").hide();
    $("#userGroupSelect").show();
